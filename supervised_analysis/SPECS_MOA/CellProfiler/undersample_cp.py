...
def main():
    ...
    specs3k_plates = ['P101382',
                    ...
                    'P101376',
                    'P101356']
    specs5k_sc_features_total = pl.read_parquet("datasets/standardized/sc_profiles_classification_specs5k_total.parquet")
    ...
    treatment_plates = specs5k_sc_features_total.filter(pl.col('moa_broad') != 'DMSO').select('Metadata_Plate').unique()

    # Step 2: Filter rows to keep only those with "DMSO" in treatment and plate values from the list
    filtered_df = specs5k_sc_features_total.filter((pl.col('moa_broad') != 'DMSO') | (pl.col('moa_broad') == 'DMSO') & (pl.col('Metadata_Plate').is_in(treatment_plates['Metadata_Plate'])))
    ...
    gc.collect()
    print("Save to file")
    resampled_specs5k_big.write_parquet("datasets/standardized/specs5k_undersampled_moa_CP.parquet")
