...
def basic_analysis(input_dir, output_file, compound_col, sample_size, permut_num):
    ...
    print("Data imported")
    anndata_group = equal_subsampling(adata, "Metadata_cmpdName", N_min= sample_size)

    print("Running etest!")
    df = etest(anndata_group, obs_key=compound_col, obsm_key='X_pca', dist='sqeuclidean', control='DIMETHYL SULFOXIDE', alpha=0.05, runs=permut_num, n_jobs=-1)
    print(df)
    print("Saving test plots.")
    
    df.loc[df.index=='DIMETHYL SULFOXIDE', 'significant_adj'] = 'DIMETHYL SULFOXIDE'
    df['neglog10_pvalue_adj'] = -np.log10(df['pvalue_adj'])
    with sns.axes_style('whitegrid'):
        sns.scatterplot(data=df, y='neglog10_pvalue_adj', x='edist', hue='significant_adj', palette={True: 'tab:green', False: 'tab:red', 'DIMETHYL SULFOXIDE': 'tab:orange'}, s=30)
    ...
    df2 = pd.DataFrame(df)
    #bool_map = {
    #'DIMETHYL SULFOXIDE': False,
    # Add other mappings as necessary
    #}
    #df2['significant_adj'] = df2['significant_adj'].map(bool_map).astype(bool)
 
    df.to_csv(f"etest_res_grit_filt_{sample_size}_samples_{permut_num}_perms.csv")
