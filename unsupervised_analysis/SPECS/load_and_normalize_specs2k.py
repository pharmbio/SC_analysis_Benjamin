...
def main():
    ...
    PROJECT_ROOT = "/share/data/analyses/benjamin/Single_cell_project/DP_specs2k/"
    REG_PARAM = 1e-2
    print("Reading Metadata")
    meta = pd.read_csv(os.path.join(PROJECT_ROOT, "inputs", "metadata", "metadata_deepprofilerspecs2k.csv")).drop_duplicates(inplace = False)
    ...
    meta_pl = pl.DataFrame(meta).drop("ER", "RNA", "AGP", "DNA", "Mito")
    meta_pl = meta_pl.unique()

    validation = meta_pl

    plates = validation["Metadata_Plate"].unique()
    plates = [p for p in plates if p not in ["P101384", "P101385", "P101386", "P101387"]]
    ...
    for p in tqdm.tqdm(plates):
        ...
        if file_path is not None:
            try:
                ...
                temp_processed.write_parquet(os.path.join("/share/data/analyses/benjamin/Single_cell_project_rapids/SPECS2K/deepprofiler/Results/", f'sc_profiles_normalized_specs2k_{p}.parquet'))
...
def prep_data(df1, features, meta_features, plate):
    mask = ~df1['Metadata_cmpdName'].is_in(['BLANK', 'UNTREATED', 'null'])
    filtered_df = df1.filter(mask)
    data_norm = normalize_pl(profiles = filtered_df, features =  features, meta_features = meta_features, samples = "Metadata_cmpdName", value= "[DMSO]", method = "mad_robustize")
