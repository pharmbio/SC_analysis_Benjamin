...
def main():
    ...
    # Start the monitor_memory function in a separate thread
    memory_thread = threading.Thread(target=monitor_memory, args=(5,), daemon=True)
    memory_thread.start()

    print("Importing data")
    
    mad_norm_df = pl.read_parquet('/home/jovyan/share/data/analyses/benjamin/Single_cell_project_rapids/Beactica/Results/sc_profiles_all_sampled_5%_BEACTICA.parquet')
    plates = list(mad_norm_df["Metadata_Plate"].unique())
    print("Data imported")

    replicate_group_grit = {
    "profile_col": "Metadata_Cell_Identity",
    "replicate_group_col": "compound_id",
    }
    features = [col for col in mad_norm_df.columns if "Feature" in col]
    meta_features = [ col for col in mad_norm_df.columns if col not in features] + ["Metadata_Cell_Identity"]
    
    #check if plate already analysed:
    results_directory = "/home/jovyan/share/data/analyses/benjamin/Single_cell_project_rapids/Beactica/Results/grit"
    plates_to_skip = set() 
    for file in os.listdir(results_directory):
        if file.endswith(".parquet"):  # Ensure to check only relevant .parquet files
            # Extract plate identifier from the file name, assuming it follows "sc_grit_{p}.parquet" format
            plate_id = file.split("_")[-1].split(".")[0]
            plates_to_skip.add(plate_id)

    print("Starting grit pipeline")
    for p in plates:
        if p in plates_to_skip:
            print(f"Skipping analysis for plate {p} as it is already processed.")
            continue
        print(f"Analysing plate {p}")
        all_sc_grit_results = []
        sc_df = mad_norm_df.filter(pl.col("Metadata_Plate") == p)
        sc_df = sc_df.with_columns(
        (pl.lit("cell_") + pl.arange(0, sc_df.height).cast(str)).alias("Metadata_Cell_Identity"))
        neg_contr = sc_df.filter(pl.col("compound_id") == "[DMSO]")
        sampled_neg_contr = neg_contr.sample(n = int(0.1*neg_contr.height), seed = 42)
        neg_contr_cells = list(sampled_neg_contr["Metadata_Cell_Identity"].unique())
        comp_list = list(sc_df["compound_id"].unique())

        for comp in tqdm.tqdm(comp_list, desc = "Processing compounds"):
            if comp == "[DMSO]":
                continue
            print("Analysing compound", comp)
            start_time = time.time()
            subset_df = sc_df.filter(pl.col("compound_id") == comp)
            subset_df = pl.concat([subset_df, sampled_neg_contr])
            subset_df2 = subset_df.to_pandas()
            sc_grit_result = evaluate(
                    ...
                    grit_control_perts = neg_contr_cells,
            ...
            print(f"Iteration {comp} completed in {iteration_time:.2f} seconds")

        all_sc_grit_df = pd.concat(all_sc_grit_results).reset_index(drop=True)
        all_sc_grit_df2 = all_sc_grit_df.rename(columns={'perturbation': 'Metadata_Cell_Identity'})
        #final_grit_df = all_sc_grid_df2.merge(sc_df.to_pandas(), on='Metadata_Cell_Identifier', how='left')
        sc_df.to_pandas().to_parquet(f"/home/jovyan/share/data/analyses/benjamin/Single_cell_project_rapids/Beactica/Results/grit/sc_features_{p}.parquet")
        all_sc_grit_df2.to_parquet(f"/home/jovyan/share/data/analyses/benjamin/Single_cell_project_rapids/Beactica/Results/grit/sc_grit_{p}.parquet")
        sampled_neg_contr.to_pandas().to_parquet(f"/home/jovyan/share/data/analyses/benjamin/Single_cell_project_rapids/Beactica/Results/grit/neg_control_features_{p}.parquet")
